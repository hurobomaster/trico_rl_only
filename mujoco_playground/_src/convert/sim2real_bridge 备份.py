...
def example_basic_conversion():
    ...
    try:
        ...
        # 真机的当前状态
        real_servo_angles = np.array([260.0, 170.0, 170.0, 260.0, 260.0, 170.0, 170.0, 260.0])
        print(f"\n真机舵机角 (度数): {real_servo_angles}")
        
        # 转换为仿真Hinge角
        hinge_angles = bridge.servo_angles_to_hinge_angles(real_servo_angles)
        print(f"仿真Hinge角 (弧度):")
        print(f"  右: OA=[{hinge_angles[0]:.4f}, {hinge_angles[1]:.4f}], OB=[{hinge_angles[2]:.4f}, {hinge_angles[3]:.4f}]")
        print(f"  左: OA=[{hinge_angles[4]:.4f}, {hinge_angles[5]:.4f}], OB=[{hinge_angles[6]:.4f}, {hinge_angles[7]:.4f}]")
        print(f"✓ 观测转换成功")
...
def example_ik_sync():
    ...
    try:
        ...
        # 当前物理舵机角
        current_angles = np.array([260.0, 170.0, 170.0, 260.0])
        print(f"当前右手指舵机角: {current_angles}")
        
        # 同步IK状态
        success = bridge.sync_ik_state_to_physical(current_angles, 'right')
...
if __name__ == "__main__":
    if TRICO_AVAILABLE:
        ...
        print("❌ Trico-Control not available")
