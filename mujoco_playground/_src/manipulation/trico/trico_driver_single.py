"""Single-policy-action Trico screwdriver rotation task."""
...
DEBUG_TRICO_DRIVER_SINGLE_OBS = False


class TricoDriverSingleEnv(mjx_env.MjxEnv):
    """Screwdriver rotation task with mirrored 4-D actions for both hands."""
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        # Single-contact can also generate useful torque for this hinge-only object.
        ...
        if DEBUG_TRICO_DRIVER_SINGLE_OBS:
            jax.debug.print(
                (
                    "driver pos={pos:.3f} vel={vel:.3f} "
                    "r_rot={rrot:.3f} p_tip={ptip:.3f} "
                    "contact={contact:.1f} p_delta={pdelta:.3f} "
                    "p_y={py:.3f} reward={rew:.3f}"
                ),
                pos=driver_joint_pos,
                vel=driver_joint_vel_signed,
                rrot=rotate_reward,
                ptip=penetration_penalty,
                contact=contact_both,
...
def default_config() -> config_dict.ConfigDict:
    """Default configuration for the TricoDriverSingle environment."""
